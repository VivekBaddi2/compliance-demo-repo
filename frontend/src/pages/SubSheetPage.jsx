...
const SubSheetPage = () => {
    ...
    const { type: routeType, id: routeId, companyId: routeCompanyId } = useParams();
            ...
            <div className='bg-gray-50 h-screen m-0 p-0 '>
                <header>
                    <section className='flex flex-col items-center my-2'>
                        <div>
                            <h1 className='text-xl font-extrabold'>{`${sheetType.charAt(0).toUpperCase() + sheetType.slice(1)} Sheets`}</h1>
                        </div>
                        <div>
                                            ...
                                            {sheetList.map((sheet, index) => {
                                                return (
                                                    <li key={index} className={`${sheetActive.id == sheet._id && `bg-blue-500/90 p-2 text-white`} flex items-center justify-between my-2`}>
                                                        ...
                                                        <button onClick={() => openSheetBtnClick(sheet._id)} className='bg-blue-500 py-1 px-2 rounded-md text-sm border border-blue-500 text-white font-semibold'>Open Sheet</button>
                                                    </li>
                                                )
                                            })}
                                        </ul>
